...
async function saveToSheet(form, code, total) {
  try {
    ...
    await fetch(`${SHEET_WEBHOOK_URL}?${params.toString()}`, { method:"GET", mode:"no-cors" });
  } catch(_) {}
}
...
.hero-cta{display:inline-block;margin-top:20px;background:var(--o);color:#fff;
  ...
  border-radius:12px;border:none;cursor:pointer;box-shadow:0 4px 16px rgba(224,123,0,.35);transition:all .2s}
...
.how-step{background:var(--w);border:1.5px solid var(--bd);border-radius:14px;padding:16px 14px}
...
.human-bar{background:var(--lo);border-left:4px solid var(--o);border-radius:10px;
  padding:14px 18px;margin:0 16px;max-width:648px;margin-left:auto;margin-right:auto;
...
.nodoes-section{background:var(--lr);border:1.5px solid rgba(192,57,43,.2);
  border-radius:14px;padding:18px 20px;margin:0 16px;max-width:648px;margin-left:auto;margin-right:auto}
...
.step{flex:1;padding:9px 5px;text-align:center;font-size:10px;font-weight:500;color:var(--mid);
  background:var(--lite);border-right:1px solid var(--bd);display:flex;align-items:center;justify-content:center;gap:3px;line-height:1.3}
...
.pay-dot{width:18px;height:18px;border-radius:50%;border:2px solid var(--bd);flex-shrink:0;position:relative;transition:all .2s}
.pay-opt.sel .pay-dot{border-color:var(--g);background:var(--g)}
.pay-dot::after{content:'';width:6px;height:6px;border-radius:50%;background:#fff;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);opacity:0;transition:opacity .2s}
...
export default function KoyaMVP() {
  ...
  function setDeparture(v) { setForm(f => ({...f, from: v, to: ""})); setErr(""); }

  function validate() {
    if (step === 1) {
      if (!form.from) return "Choisis une ville de départ.";
      if (!form.to)   return "Choisis une destination.";
      if (!line)      return "Ce trajet n'est pas encore disponible. Contacte-nous sur WhatsApp.";
  ...
  return (
    <>
      ...
      <div className="hero">
        ...
        <p className="hero-sub">On s'occupe de trouver ta place et de la sécuriser pour toi.</p>
      ...
      <div className="confiance-band">
        <div className="confiance-inner">
          {["Place garantie","Départ confirmé","Compagnie fiable sélectionnée automatiquement","Assistance KOYA jusqu'au départ"].map((item,i) => (
            <div key={i} className="conf-item"><div className="conf-check">✓</div>{item}</div>
          ))}
        </div>
      </div>
      ...
      <div className="price-band">
        <div className="price-band-inner">
          ...
          <div className="price-band-right">Tu paies uniquement une fois ta place confirmée.</div>
        </div>
      </div>
                ...
                <div className="row">
                  ...
                  <div className="field">
                    ...
                    <select value={form.to} onChange={e => set("to",e.target.value)} disabled={!form.from}>
                      <option value="">{form.from ? "Choisir..." : "Choisis le départ d'abord"}</option>
                  ...
                  <div className="pref-grid">
                    {PREFS.map(p => (
                      <div key={p} className={`pref-opt ${form.pref===p?"sel":""}`}
                        onClick={() => set("pref", form.pref===p?"":p)}>{p}</div>
                    ))}
                  </div>
                </div>
            ...
            {step === 2 && (
              <>
                ...
                <div className="field">
                  ...
                  <textarea placeholder="Ex : bagages volumineux, voyage avec enfant, personne âgée..."
                    value={form.besoin} onChange={e => set("besoin",e.target.value)} />
                </div>
              ...
              <div className="success-wrap">
                ...
                <div className="val-steps">
                  ...
                  <div className="val-step">
                    ...
                    <div className="val-text"><strong>Demande préparée</strong><span>{code}</span></div>
                  </div>
                  <div className="val-step">
                    <div className="val-num vs-wait">→</div>
                    <div className="val-text"><strong>Tu envoies sur WhatsApp</strong><span>Clique le bouton ci-dessous — étape obligatoire</span></div>
                  </div>
                  <div className="val-step">
                    <div className="val-num vs-wait">⏳</div>
                    <div className="val-text"><strong>KOYA sélectionne une compagnie et vérifie ta place</strong><span>Preuve de disponibilité obligatoire avant confirmation</span></div>
                  </div>
                  <div className="val-step">
                    <div className="val-num vs-next">→</div>
                    <div className="val-text"><strong>Confirmation + paiement sur WhatsApp</strong><span>Tu paies seulement après confirmation réelle</span></div>
                  </div>
                </div>
                ...
                <div className="wa-mandatory">⚠️ Sans envoi WhatsApp, KOYA ne reçoit pas ta demande.</div>
